...
def parser_corrections_file(filepath):
...
def parser_deletes_file(filepath):
    print ('parser_deletes_file')
    removes = []
    with open(filepath) as f:
        contents = f.readlines()
        try:
...
def main(args):
    ...
    if not corrections_file=='':
        corrections_dict = parser_corrections_file(corrections_file)
        
    if not corrections_file=='':
        deletes = parser_deletes_file(deletes_file)
